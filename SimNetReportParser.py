...
def readLessonFile(file):
   ...
   if(file != '' and file.rfind('.csv') != -1):
      ...
      ourDialect = csv.Sniffer().sniff(csvfile.read(1024))
      csvfile.seek(0)
      ...
      for line in reader:
         ...
         #StudentID,LastName,FirstName,Title,Date,NumberComplete,TotalTasks,PercentComplete
         #    0    ,  1     ,    2    ,  3  , 4  ,      5       ,     6    ,      7
         if lineNo > 0:
            ...
            if(not str(line[0]) in percent.keys()):
               percent[str(line[0])] = {}
            percent[str(line[0])][key] = line[7]  # Store percent by ID and title.
...
def writeCombinedFile(file, lessonInfo, examInfo, takeHighestExam, selectPointsOrCorrect, missingScoreMark = ""):
   # PRE-PROCESS:  Sort the student names list and exam names list:
      #First make sure we have the 'names' key in both examInfo and lessonInfo;
      ...
      d = {}
      for k in examInfo['names'].keys(), lessonInfo['names'].keys():
         for x in k:
            d[x] = 1
      sortedNames = d.keys()
      del d
      ...
      if(file != ''):
         ...
         for name in sortedNames:
            ...
            for key in sortedLessonTitles:
               if(key in lessonInfo['percent'][SID].keys()):
                  outputrow.append(lessonInfo['percent'][SID][key])
               else:
                  outputrow.append(missingScoreMark)

            # Get the list of Percent Points in order for this title:
            ppts = {}
            for key in sortedExamTitles:
               ppts[key] = {}
...
class SNRParser(Frame):
   ...
   def createWidgets(self):
      instText = "Choose an Exam and/or Lesson report below\n"
      ...
      self.missingScoreValueBox.grid(column=2,row=5, sticky=W)
      self.missingScoreLabel = Label(self, text="Insert this value for missing scores:")
      self.missingScoreLabel.grid(column=0,row=5,sticky=W, columnspan=2)

      self.goButton = Button ( self, text="Generate!",command=self.generate, state=DISABLED)
      self.goButton.grid(columnspan=3, row=6, rowspan=2, sticky=S, pady=15)
   ...
   def generate(self):
      ...
      if(writeCombinedFile(outputFileName, lessonInfo, examInfo, self.examTakeHighestAttempt.get(), self.usePctPoints.get(), self.missingScoreValueBox.get())):
